...
function load_wine(_search){
 jeedom.wcellar.wine.all({
   ...
   success: function (data) {
    ...
    var ul = '';
    for(var i in data){
        ul += '<li class="li_wine" data-wine_id="'+data[i].id+'"><a style="font-size:0.9em;">'+data[i].region+' - '+$.trim(data[i].name+' '+data[i].producer)+' - '+data[i].color+' ('+data[i].numberOfBottle+')</a></li>';
    }
    $('#ul_wine').append(ul);
